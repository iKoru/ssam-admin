        ...
        <v-card id="notificationCard" class="mb-5">
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12>
                  <v-text-field name="userId" v-model="editedItem.userId" label="ID" placeholder="입력하면 해당 회원에게만, 미입력하면 전체 회원에게 알림"></v-text-field>
                </v-flex>
                <v-flex xs12>
                ...
                <v-flex xs12>
                  <v-text-field name="target" v-model="editedItem.target" label="타겟" hint="알림 변경 시 식별할 수 있는 값"></v-text-field>
                </v-flex>
              </v-layout>
            </v-container>
          </v-card-text>
