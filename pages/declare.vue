        ...
        <v-data-table :headers="headers" :items="users" id="userTable" :rows-per-page-items="[15]" :loading="loading" :total-items="totalUsers" :pagination.sync="pagination" :no-data-text="noresult">
          ...
          <template slot="actions-prepend">
            <v-dialog v-model="dialog" max-width="500px" :fullscreen="$vuetify.breakpoint.smAndDown">
              <v-btn slot="activator" color="primary" dark class="mb-2">신규 회원 제재 생성</v-btn>
              <v-card>
                <v-card-title>
                ...
                <v-card-text>
                  <v-container grid-list-md>
                    <v-layout wrap>
                      <v-flex xs12>
                        <v-text-field name="userId" v-model="editedItem.userId" label="ID" :readonly="formTitle !== '회원 제재 생성'" :required="formTitle === '회원 제재 생성'"></v-text-field>
                      </v-flex>
                      <v-flex xs12 sm6>
                        <v-text-field name="email" v-model="editedItem.email" label="이메일"></v-text-field>
                      </v-flex>
                      <v-flex xs12 sm6>
                        <v-text-field name="password" type="password" v-model="editedItem.password" label="비밀번호" :required="formTitle === '회원 제재 생성'" :placeholder="formTitle !== '회원 제재 생성'?'입력하면 변경, 비우면 변경안함':null"></v-text-field>
                      </v-flex>
                      <template v-if="formTitle !== '회원 제재 생성'">
                        <v-flex xs12 sm6>
                      ...
                      <v-flex xs12>
                        <v-textarea name="memo" v-model="editedItem.memo" label="회원 제재 메모" placeholder="관리자만 볼 수 있는 회원 제재의 메모입니다." hint="관리자만 볼 수 있는 회원 제재의 메모입니다."></v-textarea>
                      </v-flex>
                    </v-layout>
                  </v-container>
                </v-card-text>
                ...
                <v-card-actions class="pa-3">
                  <template v-if="formTitle !== '회원 제재 생성'">
                    <v-btn color="error" @click="deleteItem(editedItem)">회원 제재 삭제</v-btn>
                    <v-spacer></v-spacer>
                  </template>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "회원 제재 생성" : "회원 제재 수정";
    }
  },
  ...
  created: async function() {
    alert('제재에 대한 정확한 요건이 나오는대로 만들게요.')
  },
  ...
  methods: {
    ...
    deleteItem: async function(item) {
      ...
      if(index < 0){
        alert('선택된 회원 제재이 없습니다. 확인 후 다시 시도해주세요.');
        return;
      }
      if (confirm("정말 이 회원 제재을 삭제하시겠습니까? 해당 회원 제재이 작성한 글 등은 유지되고, 소유한 토픽이 있을 경우 삭제가 불가능합니다.")) {
        this.loading = true;
        let response;
        try {
          ...
          this.loading = false;
          if(err.response){
            this.$router.app.$emit("showSnackbar", `회원 제재을 삭제하지 못했습니다.[${err.response.data.message}]`, "error");
          }else{
            this.$router.app.$emit('showSnackbar', '서버가 구동중이지 않거나 인터넷 연결이 끊어졌습니다.', 'error');
          }
          return;
        }
        this.$router.app.$emit("showSnackbar", `${this.users[index].userId} 회원 제재을 삭제하였습니다.`, "success");
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          ...
          if(!err.response){
            ...
            this.$router.app.$emit("showSnackbar", `회원 제재정보를 수정하지 못했습니다.[${err.response.data.message}]`, "error");
            return;
          }
        }
        ...
        try{
          ...
          if(err.response){
            this.$router.app.$emit("showSnackbar", `회원 제재 공통정보 수정완료 후 전공/지역/학년/그룹 정보를 수정하는 도중 오류가 발생했습니다.[${err.response.data.message}]`, 'error')
          }else{
            this.$router.app.$emit('showSnackbar', '서버가 구동중이지 않거나 인터넷 연결이 끊어졌습니다.', 'error');
          }
          return;
        ...
        this.$router.app.$emit("showSnackbar", `${this.users[this.editedIndex].userId} 회원 제재 정보를 수정하였습니다.`, "success");
      } else {
        //create
        let response;
        try {
          ...
          if(err.response){
            this.$router.app.$emit("showSnackbar", `회원 제재을 추가하지 못했습니다.[${err.response.data.message}]`, "error");
          }else{
            this.$router.app.$emit('showSnackbar', '서버가 구동중이지 않거나 인터넷 연결이 끊어졌습니다.', 'error');
          }
          return;
        ...
        this.$router.app.$emit("showSnackbar", `${this.editedItem.userId} 회원 제재을 추가하였습니다.`, "success");
      }
      this.close();
    },
